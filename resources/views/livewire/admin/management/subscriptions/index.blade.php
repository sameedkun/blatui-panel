...
<div class="flex flex-col gap-6">
    ...
    {{-- Page header --}}
    <x-admin.page-header title="Subscriptions" description="Every subscription ever sold, across every user and plan."
        :breadcrumbs="[['label' => 'Home', 'url' => route('admin.dashboard')], ['label' => 'Subscriptions']]" />
    ...
    <x-admin.filter-bar :config="$filterBarConfig" :filters="$filters" :has-active-filters="$this->hasActiveFilters()"
        search-placeholder="Search user, email, or plan..." />
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="border-b border-border bg-muted/40">
                    ...
                    <th class="px-4 py-3 text-left font-medium text-foreground">Plan</th>
                    ...
                    <tr wire:key="subscription-row-{{ $subscription->id }}" class="group hover:bg-muted/30">
                        ...
                        <td class="px-4 py-3">
                            @if ($subscription->plan)
                                ...
                                @endif
                            @else
                                <span class="text-muted-foreground">Deleted plan</span>
                            @endif
                                ...
                                @can('subscriptions.manage')
                                    <x-admin.tooltip text="View subscription">
                                        <x-ui.button variant="ghost" size="icon" class="size-8" href="{{ route('admin.subscriptions.show', $subscription) }}">
                                            ...
                                            <span class="sr-only">View subscription</span>
                                        </x-ui.button>
                                    </x-admin.tooltip>
                                @endcan
