{{--
    Per-record audit trail — shared by the Users and Guests profile pages (same
    reuse pattern as the placeholder tab). Reads straight off Spatie's
    ...
    dialog the full Activity Logs viewer uses. "View full history" still links
    out to that page for filtering/exporting across this record's whole trail.
...
<x-ui.card>
    <div class="mb-4 flex items-center justify-between">
        <p class="text-sm font-medium">{{ __('users.tabs.activity') }}</p>
        @can('activity_logs.view')
            <a href="{{ route('admin.activity-logs.index', ['subjectType' => $record->getMorphClass(), 'subjectId' => $record->id]) }}"
                ...
                {{ __('activity_logs.actions.view_full_history') }}
            </a>
        @endcan
    </div>
    ...
    <div class="divide-y divide-border">
        @forelse ($activities as $activity)
            ...
            <button type="button" wire:click="viewActivityDetail({{ $activity->id }})"
                wire:key="record-activity-{{ $activity->id }}"
        ...
        @empty
            <div class="flex flex-col items-center justify-center gap-2 py-12 text-center">
                ...
                <p class="text-sm text-muted-foreground">{{ __('activity_logs.status_labels.no_activity') }}</p>
            </div>
        @endforelse
    </div>
