@php
    ...
    $dates = [
        ['label' => __('guests.fields.registered'), 'value' => $record->registration_date],
        [
            'label' => __('guests.fields.last_login'),
            'value' => $record->last_login,
            'diff' => true,
            'fallback' => __('guests.status_labels.never'),
        ],
        ['label' => __('common.created_at'), 'value' => $record->created_at],
    ];

    $bool = fn(bool $v): string => $v ? __('common.yes') : __('common.no');
    $status = [['label' => __('guests.status_labels.banned'), 'value' => $bool($record->isBanned())]];
@endphp
        ...
        <div
            ...
            @can('guests.manage')
                <x-ui.button variant="outline" size="sm" wire:click="openAssignPlanDialog"
                    ...
                    <span>{{ __('guests.actions.assign_plan') }}</span>
                </x-ui.button>
            @endcan
        </div>
    @endif
