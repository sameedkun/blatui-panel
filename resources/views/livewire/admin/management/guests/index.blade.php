<div class="flex flex-col gap-6">
    ...
    {{-- Toolbar --}}
    <x-admin.filter-bar :config="$filterBarConfig" :filters="$filters" :has-active-filters="$this->hasActiveFilters()" :search-placeholder="__('common.search')" />
...
$canBulkAct = auth()
    ->user()
    ->canAny(['guests.ban', 'guests.unban', 'guests.delete']);
$canRowAct = auth()
    ->user()
    ->canAny(['guests.ban', 'guests.delete', 'guests.restore', 'guests.force-delete']);
    @endphp
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="border-b border-border bg-muted/40">
                    <th class="w-10 px-4 py-3 text-left">
                        @if ($canBulkAct)
                            <input type="checkbox" x-data
                                :checked="@js($pageIds).length > 0 && @js($pageIds).every(id =>
                                    $wire
                    ...
                    <th class="hidden px-4 py-3 text-left font-medium text-foreground xl:table-cell">
                        {{ __('guests.fields.last_login') }}</th>
                    ...
                    <tr wire:key="user-row-{{ $user->id }}"
                        ...
                        <td class="px-4 py-3">
                            @if ($canBulkAct && !$user->trashed())
                                <input type="checkbox" x-data
                                    :checked="$wire.selectedIds.includes('{{ $user->id }}')"
                        ...
                        <td class="px-4 py-3">
                            <div class="flex flex-wrap items-center gap-1.5">
                                ...
                                @if ($user->banned_at)
                                    <x-ui.badge
                                        variant="destructive">{{ __('guests.status_labels.banned') }}</x-ui.badge>
                                @elseif (!$user->trashed())
                                            ...
                                            @can('guests.ban')
                                                @if (!$user->banned_at)
                                                    <x-admin.dropdown-item
                                                        @click="$wire.openBanDialog({{ $user->id }})">
    ...
    @if (count($selectedIds) > 0)
        <div class="fixed bottom-6 left-1/2 z-50 flex -translate-x-1/2 items-center gap-1 rounded-full border border-border bg-background px-3 py-2 shadow-xl"
            ...
            <span class="px-1 text-sm font-medium">{{ count($selectedIds) }}
                {{ __('guests.status_labels.selected') }}</span>
            <div class="mx-1 h-4 w-px bg-border"></div>
