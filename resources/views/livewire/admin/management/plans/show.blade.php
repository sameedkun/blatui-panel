<div class="flex flex-col gap-6">
    ...
    {{-- Hero Plan Banner Header --}}
    @include('livewire.admin.management.plans.show.components.header', ['record' => $record])

    {{-- Plan Performance & Metrics Dashboard Grid --}}
        ...
        @foreach ($stats as $stat)
            <div class="relative overflow-hidden rounded-xl border border-border/80 bg-card p-4 shadow-2xs transition-all hover:border-border">
                <div class="flex items-center justify-between gap-2">
                    <p class="text-xs font-medium text-muted-foreground truncate">{{ $stat['label'] }}</p>
    ...
    {{-- Shared single-row action dialogs --}}
