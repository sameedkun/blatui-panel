<div class="max-w-2xl space-y-4 border-t border-border pt-6">
    <div>
        <h4 class="text-sm font-semibold">{{ __('settings.mail.test.title') }}</h4>
        <p class="text-sm text-muted-foreground">{{ __('settings.mail.test.description') }}</p>
    </div>

    <div class="flex flex-col gap-4 sm:flex-row sm:items-end">
        <x-ui.field class="flex-1">
            <x-ui.field-label for="test_email_address">{{ __('settings.mail.test.send_to') }}</x-ui.field-label>
        ...
        @if (config('mail.default') === 'resend')
            <x-ui.field class="sm:w-52">
                <x-ui.field-label for="test_email_purpose">{{ __('settings.mail.test.purpose') }}</x-ui.field-label>
