{{--
    Per-record audit trail for a ticket. Reads straight off Spatie's
...
<x-ui.card>
    <div class="mb-4 flex items-center justify-between">
        <p class="text-sm font-medium">{{ __('tickets.activity.title') }}</p>
        @can('activity_logs.view')
            <a href="{{ route('admin.activity-logs.index', ['subjectType' => $record->getMorphClass(), 'subjectId' => $record->id]) }}"
                ...
                {{ __('tickets.activity.view_full_history') }}
            </a>
        @endcan
    </div>
    ...
    <div class="divide-y divide-border">
        @forelse ($activities as $activity)
            ...
            <button type="button" wire:click="viewActivityDetail({{ $activity->id }})"
                wire:key="ticket-activity-{{ $activity->id }}"
        ...
        @empty
            <div class="flex flex-col items-center justify-center gap-2 py-12 text-center">
                ...
                <p class="text-sm text-muted-foreground">{{ __('tickets.empty.activity') }}</p>
            </div>
        @endforelse
    </div>
